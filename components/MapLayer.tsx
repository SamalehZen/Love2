...
const MapLayer: React.FC<MapLayerProps> = ({
  ...
  useEffect(() => {
    ...
    const map = window.L.map(mapRef.current, {
      ...
      zoomSnap: 0.1,
    }).setView([11.588, 43.145], 13);

    mapInstance.current = map;

    window.L.tileLayer(MAP_STYLE_DARK, {
      maxZoom: 20,
      opacity: 0.9,
    }).addTo(map);
  ...
  useEffect(() => {
    ...
    userMarkersRef.current.forEach((marker) => map.removeLayer(marker));
  ...
  useEffect(() => {
    ...
    placeMarkersRef.current.forEach((marker) => map.removeLayer(marker));
  ...
  useEffect(() => {
    ...
    if (selectedUser) {
      map.flyTo([selectedUser.location.lat, selectedUser.location.lng], 17, {
        animate: true,
        duration: 1.5,
        easeLinearity: 0.25,
      });
    ...
    if (!users.length && !places.length) return;

    const bounds = L.latLngBounds();
    users.forEach((user) => bounds.extend([user.location.lat, user.location.lng]));
  ...
  }, [users, places, selectedUser, selectedPlace]);

  return <div ref={mapRef} className={`w-full h-full absolute inset-0 z-0 ${className || ''}`} />;
};
